...
const extractPrimaryKey = (primaryKey) => (payload) => {
    if (F.isFunction(primaryKey)) {
        return primaryKey(payload);
    }
    return F.get(primaryKey, payload);
};
...
const create = ({
    ...
    callAPI,
    shouldCallAPI,
    primaryKey = ['id'], // function or array (path)
    ...
    const getPrimaryKey = extractPrimaryKey(primaryKey);

    const actionHandlers = {
        [requestType]: (state, action) => {
            const _id = getPrimaryKey(action.payload);
